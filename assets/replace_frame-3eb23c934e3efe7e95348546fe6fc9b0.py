...
if __name__ == "__main__":
    ...
    tb_xml = open("tutorialbackChunk.xml", 'r').read()
    tf_xml = open("tutorialfrontChunk.xml", 'r').read()
    
    dir = 'portraits'

    for i in range(0,100):

        out_fname_tutorial = "%02it.svg" % (i+1)
        ...
        with open(os.path.join(dir, target_svg), 'r') as infile, \
             open(os.path.join(dir, out_fname_tutorial), 'w') as outfile_m:

            fh = FrameHandler(tb_xml, tf_xml, False)
            fh.out_svg = outfile_m
            xml.sax.parse(infile, fh)
